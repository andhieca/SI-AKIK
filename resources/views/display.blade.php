...
<body class="h-screen w-screen overflow-hidden text-gray-800" 
    ...
    <!-- Main Container -->
    <div class="h-full w-full flex flex-col relative bg-cover bg-center bg-no-repeat" style="background-image: url('https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80');">
        
        <!-- Overlay backdrop -->
        <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-[2px]"></div>

        <!-- Header -->
        <header class="relative z-10 glass-panel border-b border-white/20 p-4 shrink-0 flex justify-between items-center bg-white/80">
            <div class="flex items-center gap-4">
                <img src="{{ asset('images/logo-kab-bandung.png') }}" onerror="this.src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Lambang_Kabupaten_Bandung%2C_Jawa_Barat%2C_Indonesia.svg/1200px-Lambang_Kabupaten_Bandung%2C_Jawa_Barat%2C_Indonesia.svg.png'" alt="Logo" class="h-16 w-auto drop-shadow-md">
                <div>
                    <h1 class="text-3xl font-extrabold text-bedas-800 tracking-tight">Display Monitoring SI-AKIK</h1>
                    <p class="text-bedas-600 font-medium text-lg">Pemerintah Kabupaten Bandung - Tahun Anggaran {{ session('tahun_anggaran', date('Y')) }}</p>
                </div>
            </div>
            <div class="flex flex-col items-end gap-1">
                <div class="text-3xl font-extrabold text-gray-800 tabular-nums animate-pulse" x-text="currentTime"></div>
                <div class="text-lg font-semibold text-gray-600" x-text="currentDate"></div>
                <a href="{{ route('dashboard') }}" class="text-xs text-bedas-600 mt-1 hover:underline font-semibold bg-white/50 px-3 py-1 rounded-full">< Kembali ke Dashboard</a>
            </div>
        </header>
        ...
        <main class="relative z-10 flex-grow p-8 overflow-hidden flex items-center justify-center">
            ...
            <div x-show="currentSlide === 0" 
                ...
                <h2 class="text-4xl font-extrabold text-white drop-shadow-lg text-center mb-4">Ringkasan Anggaran Utama</h2>
                
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 h-full items-center">
                    <!-- Total Anggaran -->
                    <div class="glass-panel p-8 rounded-3xl h-80 flex flex-col justify-center items-center transform hover:scale-105 transition-transform duration-500 bg-white/90">
                        ...
                        <h3 class="text-5xl font-black text-gray-800">Rp {{ number_format($totalAnggaran, 0, ',', '.') }}</h3>
                    </div>
                    ...
                    <div class="glass-panel p-8 rounded-3xl h-80 flex flex-col justify-center items-center transform hover:scale-105 transition-transform duration-500 bg-white/90">
                        <div class="bg-green-100 text-green-600 p-6 rounded-2xl mb-6 shadow-inner relative">
                            ...
                            <div class="absolute -top-3 -right-6 bg-green-500 text-white text-lg font-bold px-3 py-1 rounded-full shadow-lg">
                        ...
                        <h3 class="text-5xl font-black text-gray-800">Rp {{ number_format($totalRealisasi, 0, ',', '.') }}</h3>
                    </div>

                    <!-- Sisa Kas -->
                    <div class="glass-panel p-8 rounded-3xl h-80 flex flex-col justify-center items-center transform hover:scale-105 transition-transform duration-500 bg-white/90">
                        <div class="bg-amber-100 text-amber-600 p-6 rounded-2xl mb-6 shadow-inner">
                            <svg class="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"></path></svg>
                        </div>
            ...
            <div x-show="currentSlide === 1" 
                ...
                <h2 class="text-4xl font-extrabold text-white drop-shadow-lg text-center mb-8">Realisasi per Program</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-max overflow-hidden h-full">
                    @forelse($realisasiPerProgram->take(6) as $item)
                        <div class="glass-panel rounded-2xl p-6 flex flex-col justify-between bg-white/95 border-b-4 {{ $item['persentase'] >= 80 ? 'border-b-green-500' : ($item['persentase'] >= 50 ? 'border-b-blue-500' : 'border-b-amber-500') }}">
                            <div>
                                <div class="flex justify-between items-start mb-3">
                                    <span class="bg-gray-100 text-gray-600 py-1 px-2 rounded-md font-mono text-sm font-semibold tracking-wider">{{ $item['kode'] }}</span>
                                    <span class="text-2xl font-black {{ $item['persentase'] >= 80 ? 'text-green-500' : ($item['persentase'] >= 50 ? 'text-blue-500' : 'text-amber-500') }}">{{ number_format($item['persentase'], 1, ',', '.') }}%</span>
                                </div>
                                <h4 class="text-xl font-bold text-gray-800 mb-4 line-clamp-3 overflow-hidden text-ellipsis">{{ $item['uraian'] }}</h4>
                            </div>
                            
                            <div class="pt-4 border-t border-gray-100 mt-auto">
                                <div class="w-full bg-gray-200 rounded-full h-4 mb-4 overflow-hidden shadow-inner">
                                    <div class="h-4 rounded-full {{ $item['persentase'] >= 80 ? 'bg-gradient-to-r from-green-400 to-green-600' : ($item['persentase'] >= 50 ? 'bg-gradient-to-r from-blue-400 to-blue-600' : 'bg-gradient-to-r from-amber-400 to-amber-500') }} transition-all duration-1000 relative overflow-hidden" style="width: {{ min($item['persentase'], 100) }}%">
                                        <div class="absolute top-0 left-0 bottom-0 right-0" style="background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.4) 50%, transparent 100%); animation: shimmer 2s infinite;"></div>
                                    </div>
                                </div>
                                <div class="flex flex-col gap-1 text-sm text-gray-500">
                                    <div class="flex justify-between">
                                        <span>Realisasi</span>
                                        <span class="font-bold text-gray-800 text-lg">Rp {{ number_format($item['realisasi'], 0, ',', '.') }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Pagu</span>
                    ...
                    @empty
                        <div class="col-span-full text-white text-2xl text-center py-20 font-light">Tidak ada data program.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div x-show="currentSlide === 2" 
                ...
                <h2 class="text-4xl font-extrabold text-white drop-shadow-lg text-center mb-2">Statistik Kuitansi & Pencairan</h2>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
                    ...
                    <div class="glass-panel bg-white/95 rounded-3xl p-6 flex flex-col h-[500px]">
                        <h3 class="text-2xl font-bold text-gray-700 mb-6 flex items-center gap-2">
                            <svg class="w-8 h-8 text-bedas-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path></svg>
            ...
            <!-- Slide 4: Realisasi Pajak -->
             <div x-show="currentSlide === 3" 
                ...
                <h2 class="text-4xl font-extrabold text-white drop-shadow-lg text-center mb-4">Rekapitulasi Realisasi Pajak</h2>
                
                <div class="grid grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl">
                    @foreach($rekapPajak as $jenisPajak => $totalPajak)
                        <div class="glass-panel p-8 rounded-3xl flex flex-col items-center justify-center text-center transform hover:scale-110 transition-all duration-300 bg-white/95 border-t-8 border-bedas-500 shadow-2xl">
                            <span class="text-2xl font-bold text-gray-500 uppercase tracking-widest mb-4">{{ $jenisPajak }}</span>
                            <span class="text-4xl font-black text-gray-800 tracking-tight">Rp {{ number_format($totalPajak, 0, ',', '.') }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="absolute bottom-8 left-0 right-0 flex justify-center gap-3 z-20">
                <template x-for="i in 4" :key="i">
                    <button @click="changeSlide(i-1); resetTimer()" 
        ...
        <footer class="relative z-20 mt-auto shrink-0 border-t border-white/20 shadow-2xl">
            <div class="ticker-wrap h-14 flex items-center">
                <div class="ticker text-2xl font-semibold tracking-wide">
                    ...
                    <span class="ticker__item text-bedas-300">Total Anggaran: Rp {{ number_format($totalAnggaran, 0, ',', '.') }}</span>
                    <span class="ticker__item">|</span>
                    <span class="ticker__item text-green-300">Realisasi: Rp {{ number_format($totalRealisasi, 0, ',', '.') }} ({{ $totalAnggaran > 0 ? round(($totalRealisasi / $totalAnggaran) * 100, 1) : 0 }}%)</span>
                    <span class="ticker__item">|</span>
                    <span class="ticker__item text-amber-300">Sisa Kas: Rp {{ number_format($sisaKas, 0, ',', '.') }}</span>
                    <span class="ticker__item">|</span>
                    <span class="ticker__item">Pemerintah Kabupaten Bandung BEDAS!</span>
                </div>
            </div>
        </footer>
